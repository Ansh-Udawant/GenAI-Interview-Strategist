...
import { clearError, verifyEmailOTP } from "../redux/slices/authSlice";
import { api } from "../services/api";

/**
 * Registration Email Verification OTP Page Component.
 *
...
export default function VerifyEmailOTP() {
  ...
  const handleSubmit = async (e) => {
    ...
    dispatch(clearError());
    const result = await dispatch(verifyEmailOTP({ email: pendingEmail, otp }));
  ...
  const handleResend = async () => {
    try {
      setInfoMsg("");
      const response = await api.post("/api/auth/resend-verification-otp", { email: pendingEmail });
      setInfoMsg(response.data.message || "New verification code sent to your email.");
    } catch (err) {
  ...
  return (
    <div className={`min-h-screen flex flex-col items-center justify-center p-4 transition-colors duration-200 relative ${
      ...
      <div className={`w-full max-w-sm p-6 sm:p-8 rounded-xl border text-center transition-all ${
        ...
        <h1 className="text-xl font-bold tracking-tight mb-1">Verify Email Address</h1>
        <p className={`text-xs mb-6 ${isDark ? "text-zinc-400" : "text-zinc-500"}`}>
          Enter the 6-digit code sent to <strong className={isDark ? "text-white" : "text-zinc-900"}>{pendingEmail || "your email"}</strong>
        </p>
